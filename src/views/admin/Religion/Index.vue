  ...
  >
    ...
    <div class="admin-modal">
      ...
      <table>
        <tr>
          <td>Nama</td>
          <td>: {{ tempReligion.name }}</td>
        </tr>
      </table>
      <div class="admin-button-red" @click="deleteReligion">
  ...
  <div class="grid grid-cols-12 font-nunito bg-neutral-100">
    ...
    <div class="col-span-10 p-4">
      <div class="admin-card mb-8 flex justify-between">
        ...
        <router-link :to="{ name: 'ReligionCreate' }">
...
export default {
  ...
  methods: {
    getAllReligions: function () {
      ...
      instance
        ...
        .then((data) => {
          ...
          this.religions.value = data.data.data.results.map((item) => {
            return {
              id: item.id,
              name: item.name,
            };
    ...
    showDelete(id) {
      ...
      this.tempReligion = this.religions.value.find((obj) => {
        return obj.id === id;
      });
    },
    deleteReligion() {
      ...
      instance
        .post("admin/religion/" + this.tempReligion.id, {
          _method: "DELETE",
        })
        .then((data) => {
          this.isLoading = false;
